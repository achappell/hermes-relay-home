...
| Commands | Command dispatch is an explicit gateway operation (`command.dispatch`) when the capability is advertised. | Forward and render commands only when advertised and proven. A command is never smuggled into model input as a prompt. |
