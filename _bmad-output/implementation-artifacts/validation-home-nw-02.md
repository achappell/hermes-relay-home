...
- Durable keyed credential digests, AES-GCM replacement retry material,
  ...
  overlap, metadata tamper failure, re-enrollment notification, rotation, and
  revocation.
- Paired/legacy/disabled runtime modes, no static fallback, admin route
  ...
  arbitration snapshot, read-only authentication, malformed-state rejection,
  explicit-null reason rejection, revocation during arbitration, and stable
  errors.

## Review outcome

All three independent review layers ran against the complete baseline diff.
Their patch findings and the follow-up corrective findings were applied and
rechecked. Bridge-adapter coverage, enrollment cancellation,
configuration-device binding, and rotation request-ID entropy remain
explicitly deferred to the owning future slices; see the story spec's Review
Triage Log and `deferred-work.md`.
