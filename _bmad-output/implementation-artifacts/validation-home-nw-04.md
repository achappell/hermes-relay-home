...
The source under test was based on baseline commit
`750c0e1b3278bd3a2718d95b43f9f081c2ce79f1`, with the HOME-NW-04 working-tree
changes applied. No credentials or generated local state were added.

## Observed checks

| Check | Exact command | Recorded result |
| --- | --- | --- |
| Focused route/bridge/runtime suite | `uv run --no-cache --no-project --python 3.14 --with pytest --with cryptography --with 'websockets>=17,<18' -- python -m pytest -q tests/test_bridge_routes.py tests/test_bridge_endpoint.py tests/test_bridge_server.py tests/test_runtime.py` | `94 passed in 1.32s` |
| Full Home test suite | `uv run --no-cache --no-project --python 3.14 --with pytest --with cryptography --with 'websockets>=17,<18' -- python -m pytest -q` | `318 passed in 3.71s` |
